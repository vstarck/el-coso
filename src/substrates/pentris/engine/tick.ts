// Pentris tick — the whole Causality:
...
import { PENTOMINOES, pieceCells, pieceWidth } from "./pieces";
...
function rollKind(rng: RNGState): { kind: number; rng: RNGState } {
  const draw = nextRange(rng, 0, PENTOMINOES.length);
  let k = Math.floor(draw.value);
  if (k >= PENTOMINOES.length) k = PENTOMINOES.length - 1; // hi is exclusive; guard fp edge
...
function lockAndRespawn(w: SubstrateState, config: PentrisConfig, rng: RNGState): RNGState {
...
function tryRotate(w: SubstrateState, dir: number): void {
  const rot = (((w.piece_rot + dir) % 4) + 4) % 4;
  // Wall kicks: in place, then one or two cells sideways.
...
export function tickPentris(
  ...
  if (inputs.rotate !== 0) tryRotate(w, inputs.rotate > 0 ? 1 : -1);
